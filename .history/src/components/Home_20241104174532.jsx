  ...
  const handleAddCard = async (newCardData) => {
    try {
      ...
      if (newCardData.image) {
        ...
        const fileExtension = newCardData.image.name.split('.').pop();
        const fileName = `${Date.now()}_${Math.random().toString(36).substring(7)}.${fileExtension}`;
        
        const storageRef = ref(storage, `stations/${user.uid}/${fileName}`);
        
        try {
          const uploadResult = await uploadBytes(storageRef, newCardData.image);
      ...
      await addDoc(collection(db, "petrolStations"), stationData);
