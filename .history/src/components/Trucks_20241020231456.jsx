...
export default function Trucks() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Handle image upload only if it's a new image (data URL)
      if (formData.image && formData.image.startsWith("data:")) {
        ...
        // If editing and image is cleared, remove it from storage and set to null
        if (selectedTruck.image) {
          const imageRef = ref(storage, `truck-images/${selectedTruck.truckId}`);
          await deleteObject(imageRef).catch((error) => {
            console.error("Error deleting old image:", error);
          });
        }
        truckData.image = null;
      } else if (isEditing) {
        // If editing but image hasn't changed, don't update the image field
        delete truckData.image;
      }
  ...
  const handleEditClick = (truck) => {
    setFormData({
      ...
      image: truck.image, // This should be the URL, not a File object
    });
    setSelectedTruck(truck);
    setIsEditing(true);
    setShowForm(true);
