...
export default function Home() {
  ...
  const handleAddCard = async (newCardData) => {
    try {
      ...
      // Check file size
      if (newCardData.image && newCardData.image.size > 5 * 1024 * 1024) {
      ...
      if (newCardData.image) {
        try {
          ...
          const fileName = `${Date.now()}_${newCardData.image.name}`;
          const storageRef = ref(storage, `stations/${currentUser.uid}/${fileName}`);
          
          // Upload the file
          const snapshot = await uploadBytes(storageRef, newCardData.image);
          ...
          return;
        const storage = getStorage();
        const imageRef = ref(storage, `stations/${currentUser.uid}/${Date.now()}-${newCardData.image.name}`);
        await uploadBytes(imageRef, newCardData.image);
  ...
  const handleSaveEdit = async (updatedCardData) => {
    try {
      ...
      if (updatedCardData.image && updatedCardData.image instanceof File) {
        const storage = getStorage();
        const imageRef = ref(storage, `stations/${currentUser.uid}/${Date.now()}-${updatedCardData.image.name}`);
        await uploadBytes(imageRef, updatedCardData.image);
        imageUrl = await getDownloadURL(imageRef);

        if (editingCard.imageUrl) {
          const oldImageRef = ref(storage, editingCard.imageUrl);
          try {
