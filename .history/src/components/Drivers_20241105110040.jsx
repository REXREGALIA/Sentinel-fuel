...
export default function Drivers() {
  ...
  const handleEdit = async (e) => {
    ...
    try {
      const driverRef = doc(db, "drivers", editedDriver.id);
      const updateData = {
        name: editedDriver.name,
        phone: editedDriver.phone,
        address: editedDriver.address,
        licenseNo: editedDriver.licenseNo,
        vehicleNo: editedDriver.vehicleNo
      };

      await updateDoc(driverRef, updateData);
      setDrivers(drivers.map(driver => 
        driver.id === editedDriver.id ? { ...driver, ...updateData } : driver
      ));
  ...
  const handleDelete = async (driverId) => {
    try {
      if (window.confirm("Are you sure you want to delete this driver?")) {
        ...
        // Update the UI by filtering out the deleted driver
        ...
        <form onSubmit={handleEdit} className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => setEditedDriver({...editedDriver, licenseNo: e.target.value})}
              className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
            />
          </div>
          <div>
            ...
            <input
              type="text"
              value={editedDriver.vehicleNo}
              onChange={(e) => setEditedDriver({...editedDriver, vehicleNo: e.target.value})}
              className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
            />
          </div>
