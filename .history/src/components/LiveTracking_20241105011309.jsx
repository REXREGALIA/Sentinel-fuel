...
export default function LiveTracking() {
  ...
  // Function to get coordinates based on location type
  const getCoordinates = async (location) => {
    if (location.locationType === 'coordinates') {
      ...
      // For demo, using a simple mapping
      const zipCoordinates = {
        '560001': { lat: 12.9716, lng: 77.5946 },
        '560002': { lat: 12.9815, lng: 77.5921 },
        // Add more mappings as needed
      };
      return zipCoordinates[location.zipCode] || await geocodeZipCode(location.zipCode);
    }
  };

  // Function to geocode zip code (you would replace this with actual API call)
  const geocodeZipCode = async (zipCode) => {
    ...
    console.warn('Using default coordinates for zip code:', zipCode);
  ...
  const startTracking = async () => {
    ...
    try {
      ...
      // Get actual coordinates for both locations
      const [startPosition, endPosition] = await Promise.all([
        getCoordinates(selectedTruck),
        getCoordinates(selectedPump)
      ]);
        ...
        {tracking && (
          <div className="bg-gray-800 p-6 rounded-lg shadow-lg animate-fade-in">
            ...
            <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="bg-gray-700 p-4 rounded-lg">
                <h3 className="text-lg font-semibold mb-2">Start Location</h3>
                <p>{tracking.startAddress}</p>
                <p className="text-sm text-gray-400">
                  {tracking.currentPosition.lat.toFixed(4)}, 
                  {tracking.currentPosition.lng.toFixed(4)}
                </p>
              </div>
              <div className="bg-gray-700 p-4 rounded-lg">
                <h3 className="text-lg font-semibold mb-2">Destination</h3>
                <p>{tracking.endAddress}</p>
                <p className="text-sm text-gray-400">
                  {tracking.endPosition.lat.toFixed(4)}, 
                  {tracking.endPosition.lng.toFixed(4)}
                </p>
              </div>
            </div>
          </div>
