...
export default function TruckComponent() {
  ...
  const handleTruckClick = (truck) => {
    ...
    console.log(`Navigating to /truck/${truck.truckId}`)
  }

  return (
    <div className="min-h-screen bg-gray-900 text-white p-8">
      <style jsx global>{`
        @keyframes rotateFuel {
          0% { transform: rotate(0deg) translateX(40px) rotate(0deg); }
          100% { transform: rotate(360deg) translateX(40px) rotate(-360deg); }
        }
        .truck-image-container:hover .fuel-icon {
          opacity: 1;
        }
        .truck-image-container:hover .fuel-icon > svg {
          animation: rotateFuel 3s linear infinite;
        }
      `}</style>
          ...
          <div className="bg-gray-800 rounded-lg w-full max-w-2xl relative shadow-lg max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-8">
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="space-y-2">
                  <label htmlFor="lastMaintenance" className="block text-sm font-medium text-gray-300">Last Maintenance Date</label>
                  <input
                    type="date"
                    id="lastMaintenance"
        ...
        {trucks.map((truck) => (
          <div key={truck.truckId} className="flex flex-col items-center">
            <div className="truck-image-container relative w-24 h-24 sm:w-32 sm:h-32 rounded-md overflow-hidden hover:ring-4 hover:ring-blue-500 transition duration-300">
              ...
              <div className="fuel-icon absolute top-1/2 left-1/2 w-8 h-8 opacity-0 transition-opacity duration-300">
                <TruckIcon size={24} className="text-yellow-500 absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2" />
              </div>
            </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-gray-800 p-8 rounded-lg max-w-md w-full relative">
            ...
            <div className="space-y-2">
              ...
              <p><strong>Last Maintenance:</strong> {selectedTruck.lastMaintenance}</p>
            </div>
          </div>
        </div>
      )}
