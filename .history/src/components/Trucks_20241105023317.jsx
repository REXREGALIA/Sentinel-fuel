...
export default function Trucks() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!formData.truckId || !formData.model || !formData.capacity) {
        toast.error('Please fill in all required fields');
        return;
      }

      // Create base truck data
      const truckData = {
      ...
      if (formData.locationType === 'coordinates') {
        truckData.latitude = formData.latitude;
        truckData.longitude = formData.longitude;
        truckData.zipCode = '';
      } else if (formData.locationType === 'zipcode') {
        truckData.zipCode = formData.zipCode;
        truckData.latitude = '';
        truckData.longitude = '';
      }
      ...
      if (formData.image) {
        const imageRef = ref(storage, `truck-images/${formData.truckId}-${Date.now()}`);
        await uploadBytes(imageRef, formData.image);
        truckData.imageUrl = await getDownloadURL(imageRef);
      } else {
        truckData.imageUrl = ''; // Set empty string if no image
      }

      // Log the data being sent to Firestore (for debugging)
      console.log('Sending to Firestore:', truckData);
                ...
                <div className="bg-gray-800 rounded-xl w-full max-w-2xl relative shadow-2xl max-h-[90vh] overflow-y-auto">
                  ...
                  <div className="p-8">
                    ...
                    <form onSubmit={handleSubmit} className="bg-gray-800 p-6 rounded-lg mb-8">
                      ...
                      <button
                        type="submit"
                        className="w-full py-3 mt-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition duration-300 font-medium text-lg shadow-md hover:shadow-lg transform hover:-translate-y-1"
                        disabled={isLoading}
                      >
                        {isLoading
                          ? isEditing
