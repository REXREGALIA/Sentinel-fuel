    ...
    <>
      <div className="bg-white flex min-h-screen w-screen gap-10 pb-1/2">
        ...
        >
          ...
          {showForm && (
            <div className={`w-full bg-stone-100 p-4 rounded-md shadow-md flex flex-col justify-start items-end`}>
              <RxCross2 className="cursor-pointer text-2xl" onClick={handleClick}/>
              <Forms 
          ...
          <div className={`flex flex-wrap gap-10 `}>
            ...
            {cards.map((card, index) => (
              <Card cardIndex={index} id={card.name} iamge={card.image} add={card.address} onDelete={() => handleDeleteCard(index)}  onEdit={handleEditCard} />
            ))}
          </div>
